...
export const drizzleGroupRepository: GroupRepository = {
  ...
  async createInviteForMember(input) {
    return db.transaction(async (tx) => {
      // Serializing invite creation with leave/dissolution prevents an invite
      // from being created for a group while its last member is leaving.
      const [group] = await tx
        .select({ id: groups.id })
        .from(groups)
  ...
  async joinWithInvite(input): Promise<JoinGroupResult> {
    return db.transaction(async (tx) => {
      ...
      // Leave and join use the same group-row lock, so the membership count
      // cannot change between the authorization decision and the insert.
      const [group] = await tx
        .select({
          id: groups.id,
          name: groups.name,
