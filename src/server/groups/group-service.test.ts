...
class MemoryGroupRepository implements GroupRepository {
  ...
  async joinWithInvite(input: JoinGroupInput): Promise<JoinGroupResult> {
    ...
    const digest = Buffer.from(input.inviteDigest);
    if (
      !invite ||
      invite.expiresAt.getTime() <= input.now.getTime() ||
      invite.digest.byteLength !== digest.byteLength ||
      !timingSafeEqual(invite.digest, digest)
    ) {
...
describe("group service", () => {
  it("creates a group with the creator as an ordinary member", async () => {
    ...
    await assert.rejects(
      service.createGroup({ userId: "alice", name: "   ", now }),
      isServiceError("invalid_group_name"),
    );
  });
  ...
  it("authorizes invites and joins, while preventing duplicate membership", async () => {
    ...
    assert.equal(
      ...
      now.getTime() + GROUP_INVITE_LIFETIME_MS,
    );
    ...
    assert.match(secondInvite.token, /^wgi1_/);
  });
  ...
  it("rejects malformed and expired invites", async () => {
    ...
    await assert.rejects(
      service.joinGroup({
        token: invite.token,
        userId: "bob",
        now: new Date(now.getTime() + GROUP_INVITE_LIFETIME_MS + 1),
      }),
      isServiceError("invalid_invite"),
    );
  });

  it("exposes members only to members and permits only self-leave", async () => {
    const repository = new MemoryGroupRepository();
    const service = createGroupService(repository);
    const now = new Date("2026-09-17T00:00:00.000Z");
    const group = await service.createGroup({
    ...
    assert.equal("session" in details.members[0]!, false);
    await assert.rejects(
    ...
    await assert.rejects(
      service.leaveGroup({ groupId: group.id, userId: "outsider", now }),
      isServiceError("group_access_denied"),
    );
