...
describe("auth schema security constraints", () => {
  ...
  it("models flat group membership and hashed invite storage", () => {
    ...
    assert.ok(indexNames(groupMemberships).includes("group_memberships_user_id_index"));
    assert.ok(indexNames(groupInvites).includes("group_invites_selector_unique"));
    assert.ok(indexNames(groupInvites).includes("group_invites_expires_at_index"));
    assert.ok(checkNames(groups).includes("groups_name_length_check"));
    assert.ok(checkNames(groupInvites).includes("group_invites_digest_length_check"));
